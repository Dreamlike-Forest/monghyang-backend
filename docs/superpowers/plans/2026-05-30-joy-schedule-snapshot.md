...
**기술 스택:** Java 21, Spring Boot 3.5.3, Spring Data JPA 3.5.1, Jakarta Bean Validation, Flyway, JUnit, Mockito. 정확한 라이브러리 버전과 Context7 library ID는 `docs/context7-dependencies.yaml` 기준으로 확정했다.
...
- 완료 시점에 `docs/context7-dependencies.yaml`와 `docs/junit-unit-test-guide.md`가 모두 존재함을 확인했다.
- 사용자가 이 계획 실행을 명시 승인했고, 현재 브랜치 `feat/brewery-joy-schedule`에서 작업을 시작하라고 승인했다.
- 초기 baseline `./gradlew test` 실패는 사용자가 기존 테스트 코드를 수정한 뒤 재실행해 통과함을 확인했다.
- `superpowers:using-git-worktrees`를 적용해 격리 상태를 점검했고, 사용자의 명시 승인에 따라 현재 브랜치에서 진행했다.
...
- [x] **Step 1.2: `superpowers:using-git-worktrees` 적용**
...
- [x] **Step 3.1: 활성 스냅샷 조회 실패 테스트 작성**
...
- [x] **Step 3.2: `JoyWeeklyStartTimeRepository` 쿼리 추가**
...
- [x] **Step 3.3: `JoyOrderRepository` 환불 대상 조회 추가**
...
- [x] **Step 3.4: Flyway 인덱스 마이그레이션 추가**
...
- [x] **Step 4.1: 체험 생성 최초 스냅샷 실패 테스트 작성**
...
- [x] **Step 4.4: `createJoy` 스냅샷 저장 구현**
...
- [x] **Step 4.5: `updateJoySchedule` 구현**
...
- [x] **Step 5.1: 스냅샷 밖 시간 예약 실패 테스트 작성**
...
- [x] **Step 5.2: 사용자 예약 변경 실패 테스트 작성**
...
- [x] **Step 5.3: 양조장 관리자 예약 변경 회귀 테스트 작성**
...
- [x] **Step 5.4: `verifyReservation` 시그니처 정리**
...
- [x] **Step 5.7: `updateReservationByBrewery` 기존 오류 수정**
...
- [x] **Step 6.1: controller 연결 실패 테스트 또는 컴파일 검증 준비**

현재 controller 계층 테스트 구조가 없으면 과도한 새 테스트 인프라를 만들지 않는다. 서비스 테스트와 `compileJava`로 메서드 바인딩을 1차 검증한다.

- [x] **Step 6.2: `/api/brewery-priv/joy/schedule` 엔드포인트 추가**
...
- [x] **Step 7.3: SQL 로그 또는 DB 실행 계획 검증**
...
- 기존 계획의 컴파일 오류 가능성인 `joyInfoDto.joyId()` 사용을 제거하고, `verifyReservation`에 `joyId`를 별도 전달하는 방향으로 구현했다.
- 기존 계획에 없던 `updateReservationByBrewery`의 체험 ID 사용 오류와 기존 슬롯 감소 기준 오류를 작업 범위 안에서 해결했다. 이는 예약 변경 시 스냅샷 검증을 넣는 과정에서 같은 메서드를 수정해야 하므로 범위 내 결함 수정으로 처리했다.
- 환불 대상 조회는 `date(reservation)` 대신 `LocalDateTime` range 조건을 사용하도록 구현했다.
- `joy_weekly_start_time`은 새 인덱스를 추가하지 않고 기존 unique key를 사용하도록 유지했다. `joy_order` 인덱스는 체험 일정 변경 환불 대상 조회와 환불 스케줄러의 상태 조회가 같은 선두 컬럼을 공유하도록 `(joy_payment_status, joy_id, is_deleted, reservation)` 순서로 추가했다.
- `docs/context7-dependencies.yaml`와 `docs/junit-unit-test-guide.md` 존재를 확인하고, 테스트 작성은 `docs/junit-unit-test-guide.md` 기준을 따랐다.
...
- context7_library_id: `not_used` (기존 저장소 패턴과 기준 문서만으로 구현 가능했고, 추가 공식 API signature 확인이 필요하지 않았다.)
...
- 변경 파일:
  ...
  - `src/main/java/com/example/monghyang/domain/joy/repository/JoyWeeklyStartTimeRepository.java`
  - `src/main/java/com/example/monghyang/domain/joy/repository/JoyOrderRepository.java`
  - `src/main/resources/db/migration/V2605301200__add_index_for_joy_order_refund_schedule.sql`
  ...
  - `src/test/java/com/example/monghyang/domain/joy/dto/JoyScheduleDtoTest.java`
  - `src/test/java/com/example/monghyang/domain/joy/repository/JoyWeeklyStartTimeRepositoryTest.java`
...
- 구현된 동작:
  - 체험 생성 시 `JoyWeeklyStartTime` 최초 스냅샷 저장
  - 체험 일정 변경 API 및 스냅샷 교체
  - 체험 일정 변경 이후 적용일 포함 이후 `PAID` 예약의 `REFUND_REQUESTED` 전환
  - 예약 생성, 사용자 예약 변경, 양조장 관리자 예약 변경의 체험 시작 시간 스냅샷 검증
  - 양조장 관리자 예약 변경의 잘못된 체험 ID 사용과 기존 슬롯 감소 기준 오류 수정
- 검증 결과:
  - `./gradlew compileJava`: 성공
  - `./gradlew test`: 성공
  - `git diff --check`: 성공
  - `rg -n "date\\(jo\\.reservation\\)|date\\(.*reservation" src/main/java/com/example/monghyang/domain/joy src/main/java/com/example/monghyang/domain/brewery`: 새 체험 일정 환불 대상 조회에는 컬럼 함수 사용 없음. 기존 조회/양조장 일정 변경 쿼리의 `date(jo.reservation)`는 범위 제외 항목으로 남아 있음.
- 알려진 리스크와 후속 개선 후보:
  - 기존 데이터 중 `joy_weekly_start_time` 스냅샷이 없는 체험은 예약 캘린더와 예약 검증에서 선택 가능한 시작 시간이 없으므로 별도 backfill 또는 운영 데이터 보정이 필요하다.
  - 기존 양조장 일정 변경 쿼리의 `date(jo.reservation)` 제거는 별도 개선 과제로 남긴다.
