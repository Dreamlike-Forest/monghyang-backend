...
public interface CartRepository extends JpaRepository<Cart, Long> {
    ...
    // 해당 유저의 장바구니의 주문 목록 중 '삭제 처리되지 않고 품절 처리되지 않은' 요소를 조회
    @Query("select c from Cart c join fetch c.product p join fetch p.user u where c.id in :idList and c.user.id = :userId and p.isDeleted = false and p.isSoldout = false")
