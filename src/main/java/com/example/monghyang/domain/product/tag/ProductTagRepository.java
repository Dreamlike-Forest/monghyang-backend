...
public interface ProductTagRepository extends JpaRepository<ProductTag, Long> {
     ...
     * @return 인증 태그 리스트
     */
    @Query("select new com.example.monghyang.domain.tag.dto.TagNameDto(pt.product.id, pt.tags.name, pt.tags.id) from ProductTag pt where pt.product.id in :productIdList and pt.tags.category.id = (select tc.id from TagCategory tc where tc.name = '인증')")
