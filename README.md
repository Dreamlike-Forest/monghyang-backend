...
---
## ✍🏻 규칙
> [API Docs(Notion)](https://www.notion.so/API-Docs-2a46f2b589e680bd820ceba50deac89a) <br>
> [(필독) DB 스키마 관리 규칙(Flyway)](https://www.notion.so/Flyway-2af6f2b589e68026a501d4d78f45cf01)
...
> ### 빌드 시 환경 변수 주입(개발 환경)
> **데이터베이스명** : monghyang
> 1. 로컬 데이터베이스 연결: .env 파일의 로컬 DB 관련 환경 변수의 값을 자신의 로컬 환경에 맞게 수정(db url, storage path, ...)
> 2. 환경변수 파일을 ./src/main/resources 디렉토리에 넣기(위치 중요)
> 3. IDE를 통한 로컬 빌드 시 .env 파일의 'LOCAL_STORAGE_PATH' 환경변수 내용 수정하기(이미지 저장소로 활용할 디렉토리 절대경로)
> 4. 'role' 테이블에 유저 권한 튜플 삽입(종류는 아래 참고)
> 5. 'tags' 및 'tag_category' 테이블에 태그와 태그카테고리 튜플 삽입
> 6. 빌드

> ### JVM: 프로파일 지정하여 jar 파일 실행 예시(배포 환경)
> - !!주의사항!! 'setting.env', jar 파일은 **반드시 동일한 경로**에 위치해야 합니다.
> ```bash
> nohup java -jar [실행할 JAR 파일(확장자 포함)] \
